...
class ProfileSummarySerializer(serializers.ModelSerializer):
    candidate = CandidateProfileSerializer(required=False)  # candidate is optional for PATCH
    ...
    def update(self, instance, validated_data):
        ...
        # If candidate data is included, update nested object
        if candidate_data:
            for attr, value in candidate_data.items():
                setattr(instance.candidate, attr, value)
            instance.candidate.save()

        # Update the summary_text if it's present
